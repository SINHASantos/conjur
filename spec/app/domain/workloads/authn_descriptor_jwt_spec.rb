...
RSpec.describe Workloads::AuthnDescriptor, type: :model do
  describe 'initialization' do
    ...
    describe 'type is jwt' do
      let(:valid_type) { 'jwt' }
      let(:valid_service_id) { 'jwt-service-id-1' }
      let(:valid_data) { {} }
      ...
      describe "and data validating" do
        ...
        it "raises DomainValidationError for too big data" do
          invalid_data = (1..Workloads::Validating::AuthnDescriptorValidation::MAX_CLAIMS_SIZE + 1).map { |i| ["jwt_claim_key_#{i}", "jwt_claim_value_#{i}"] }.to_h
          expect {
        ...
        it "raises DomainValidationError for too long data value" do
          data["jwt_claim_key"] = "a" * (Annotations::Annotations::VALUE_LENGTH_MAX + 1)
          expect {
            described_class.new(**input.merge(data:))
          }.to raise_error(Validation::DomainValidationError,
