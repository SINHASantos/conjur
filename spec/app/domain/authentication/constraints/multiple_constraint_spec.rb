...
RSpec.describe(Authentication::Constraints::MultipleConstraint) do
  let(:inner_constraint) do
    double("InnerConstraint").tap do |d|
      allow(d).to receive(:validate).and_return(Responses::Success.new(true))
    end
  end
  ...
  context "Given MultipleConstraint initialized with 2 constraints" do
    let(:inner_constraint_second) do
      double("InnerConstraintSecond").tap do |d|
        allow(d).to receive(:validate).and_return(Responses::Success.new(true))
      end
    end
    let(:resource_restrictions) { %w[restriction_first restriction_second] }
    ...
    subject(:constraint) do
      Authentication::Constraints::MultipleConstraint.new(inner_constraint, inner_constraint_second)
    end

    context "when validating an array" do
      it "calls validate on both inner constraints by their order" do
        expect(inner_constraint).to receive(:validate).with(resource_restrictions: resource_restrictions).ordered
